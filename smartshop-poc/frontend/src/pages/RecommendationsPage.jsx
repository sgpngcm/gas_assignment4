    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "22px 16px" }}>
      ...
      <div style={{ marginTop: 16, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 14 }}>
        {recs.map((p) => (
          <div key={p.id}>
            <ProductCard p={p} />
            <div style={{ marginTop: 8, padding: 10, borderRadius: 12, border: "1px solid #eee", background: "white" }}>
              <div style={{ fontWeight: 900, opacity: 0.85 }}>Why recommended</div>
              {p.also_bought_count > 0 ? (
                <div style={{ marginTop: 8, opacity: 0.75, fontWeight: 800 }}>
                  Popular with similar shoppers: {p.also_bought_count} also bought this
                </div>
              ) : null}               
